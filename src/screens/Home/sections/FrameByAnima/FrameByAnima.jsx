...
export const FrameByAnima = () => {
  ...
  return (
    <section className="relative w-full py-24 overflow-hidden bg-black">
      {/* Background image */}
      <div
        className="absolute inset-0 w-full h-full z-0 bg-cover bg-center opacity-50"
        style={{
        ...
        <div className="flex flex-col items-center mt-16">
          ...
          <div className="relative w-64 mb-6">
            ...
            >
              ...
              {/* Position indicators with dots (original) */}
              {/* <div className="absolute bottom-1 left-0 w-full flex justify-between px-2 z-10">
                {industriesData.map((_, index) => (
                  <div
                    key={index}
                    className={`w-1 h-1 rounded-full cursor-pointer
                    ${index <= currentIndex ? 'bg-black' : 'bg-white bg-opacity-50'}`}
                    onClick={(e) => {
                      e.stopPropagation()
